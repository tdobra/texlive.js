...
class TeXLive {
  ...
  prestart() {
    const workerSrc = TeXLive.files.getContent(this.workerFolder + "pdftex.js");
    const texSrc = TeXLive.files.getContent(this.texURL);
    this.workerReady = new Promise();
    this.cmdPromises = [];
    await src;
    ...
    await workerReady;
    await texSrc;
    this.FS_createDataFile("/", "input.tex", texSrc, true, false);
  }
...
TeXLive.files = (() => {
  ...
  async function getContent(path) {
    ...
    if (pathInd === -1) {
      ...
      await fileFetch;
      if (!fileFetch.ok) { throw fileFetch.status; }
      let fileContent;
      if (path.endsWith(".js")) {
        ...
        fileContent = fileFetch.arrayBuffer();
        await fileContent;
      }
  ...
  //TODO: Check this closure is working correctly: only downloads once - might need to enclose return in an object
  return getContent;
})();
